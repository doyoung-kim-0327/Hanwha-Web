...
public class QPostDAO_Mybatis {
	...
	public int decreasecom(int pid) {
		return session.update("com.studyboard.decreasecom", pid);
	}

	public QPostDTO qselectByContent(String pcontent) {
		return session.selectOne("com.studyboard.selectbycontent", pcontent);
	}

	public int editqPost(QPostDTO qpost) {
		return session.update("com.studyboard.qedit", qpost);
	}
